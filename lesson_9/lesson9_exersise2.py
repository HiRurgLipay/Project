...
def uniondict():
    merged_dict = {}
    for key in set(first_dict.keys()).union(second_dict.keys()):
        merged_dict.setdefault(key, []).append(first_dict.get(key))
        merged_dict.setdefault(key, []).append(second_dict.get(key))
